...
const formSchema = z.object({
  name: z
    ...
    .max(50, { message: "Doctor name must be less than 50 characters" }),
  phone: z
    .string()
...
export default function AddDoctor({ trigger }: AddDoctorProps) {
    ...
    <Drawer direction="right">
      ...
      <DrawerContent className="max-w-xl">
        <DrawerHeader className="border-b flex-row w-full justify-between items-center">
          <DrawerTitle className="font-medium">Add Doctor</DrawerTitle>
          <DrawerClose className="cursor-pointer" ref={closeRef}>
            <XIcon className="size-4" />
          </DrawerClose>
        </DrawerHeader>

        <AddDoctorForm onCancel={() => closeRef.current?.click()} />
      </DrawerContent>
    </Drawer>
  );
}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Name of the doctor" {...field} />
                </FormControl>
